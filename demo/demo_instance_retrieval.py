...
def main():
    parser = argparse.ArgumentParser(
        description='Instance Retrieval in Scene',
        epilog="""
Examples:
  # Point cloud to point cloud retrieval in ScanNet
  python demo_instance_retrieval.py --query_path chair.ply --scan_id scene0001_00
  
  # Text to point cloud retrieval  
  python demo_instance_retrieval.py --query_path "red chair" --query_modality referral --scan_id scene0001_00
  
  # Cross-modal: point cloud query to RGB targets
  python demo_instance_retrieval.py --query_path chair.ply --target_modality rgb --scan_id scene0001_00
        """,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Object Retrieval Demo')
    parser.add_argument('--query_path', default='./demo_data/kitchen/scene.ply', type=str, required=False,
                       help='Path to query object data (point cloud, image, or text file)')
    parser.add_argument('--database_path', default='./embed_instance_scannet_scene0191_00.pt', type=str, required=False,
                       help='Path to database embeddings file')
    parser.add_argument('--query_modality', default='point', type=str, required=False,
                       help='Query modality: point, rgb, referral')
    parser.add_argument('--database_modality', default='point', type=str, required=False,
                       help='Database modality: point, rgb, referral, cad')
    
    parser.add_argument('--ckpt', default='/drive/dumps/multimodal-spaces/runs/new_runs/instance_crossover_scannet+scan3r+multiscan+arkitscenes.pth', type=str, required=False,
                       help='Path to model checkpoint')
    
    # Model dimensions
    parser.add_argument('--input_dim_3d', default=384, type=int, required=False)
